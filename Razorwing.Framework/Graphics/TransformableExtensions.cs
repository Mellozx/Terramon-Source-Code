...
namespace Razorwing.Framework.Graphics
{
    public static class TransformableExtensions
    {
        ...
        {
            return transformable.Delay(Math.Max(0, transformable.LatestTransformEndTime - transformable.Time.TotalGameTime.TotalMilliseconds));
        }
        ...
        /// </summary>
        /// <returns>A <see cref="TransformSequence{T}"/> to which further transforms can be added.</returns>
        public static TransformSequence<T> FadeColour<T>(this T drawable, Color newColour, double duration = 0, Easing easing = Easing.None) where T : Drawable =>
        ...
        /// </summary>
        /// <returns>A <see cref="TransformSequence{T}"/> to which further transforms can be added.</returns>
        public static TransformSequence<T> FlashColour<T>(this T drawable, Color flashColour, double duration, Easing easing = Easing.None) where T : Drawable
        {
            Color endValue = (drawable.Transforms.LastOrDefault(t => t.TargetMember == nameof(drawable.Colour)) as Transform<Color>)?.EndValue ?? drawable.Colour;
